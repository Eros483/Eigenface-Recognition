...
def image_preparation(data, n_components):
    ...
    chunk_size = len(facematrix) // multiprocessing.cpu_count()
...
def testing(address, faces, eigenfaces, weights, facelabel, faceshape, facematrix):
    test_image = cv2.imread(address, cv2.IMREAD_GRAYSCALE)
    if test_image is None:
        print("Error reading image file:", address)
    else:
        query = test_image.reshape(1, -1)
...
def main():
    ...
    testing("test_image1.pgm", faces, eigenfaces, weights, facelabel, faceshape, facematrix)
    testing("test_person_1/1.pgm", faces, eigenfaces, weights, facelabel, faceshape, facematrix)
